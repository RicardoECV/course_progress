...
$usuario = check_Login();
if($usuario["admin"]) {
  $acessos_admin = selectSQL("SELECT * FROM acessos_com_data ORDER BY id DESC");
}
...
$acessos = selectSQL("SELECT * FROM acessos_com_data WHERE id_colaborador=$usuario[id] ORDER BY id DESC");
          ...
          <table>
            ...
            <?php foreach($acessos_admin as $i => $a): ?>
              ...
              <tr>
                ...
                <td> <?= get_DadosColaborador($a["id_colaborador"]); ?> </td>
                <td> <?= date("H:i:s - d/m/Y", strtotime($a["data"])); ?> </td>
