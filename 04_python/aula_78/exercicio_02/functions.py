...
def editarProduto():
  ...
  if(num_produto >= 0 and num_produto < len(globais.produtos)):
    ...
    if(opcao == 1):
      nome = input(f"Insira o novo nome do Produto ({produto['nome']}): ")
      if (not jaExiste(nome)):
        produto["nome"] = nome 
      else: print("\nNome já existente.")
...
def vendaProduto():
  ...
  if(num_produto >= 0 and num_produto < len(globais.produtos)):
    ...
    if(num_vendas <= produto["stock"] and num_vendas > 0):
...
def toString(i, p):
  print(f"{i+1} - Nome: ({p['nome']}) Preço: ({p['preco']:.2f} €) Quantidade: ({p['stock']})")
